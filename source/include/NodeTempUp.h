...
,pool(pool_)
,m(tL)//m: número de réplicas (usado no feedback)
,ut(ut_)
{}
...
void NodeTempUp<S>::run(){
    switch (ut){
		case 1:{
			tempUp23(); //Se a taxa for menor que 0.2 (20%), ajusta a temperatura da réplica de menor temperatura (left) para tentar alcançar ~23% de aceitação — valor ideal em muitos estudos teóricos e práticos.
		}
		break;
		case 2:{
			tempUpEnergy(); //Usa a diferença de energia entre réplicas e a temperatura do par inferior para ajustar a temperatura da réplica right.Tenta distribuir uniformemente as diferenças de energia entre as réplicas.
		}
		break;
		case 3:{
			tempUpFB(); //    Usa o fluxo de troca entre réplicas (Nup, Ndown) para calcular:Derivada do fluxo (df/dT).		Densidade de troca.			E define a melhor temperatura possível para equilibrar o fluxo
...
//Calcula a densidade de fluxo entre réplicas vizinhas, útil para o método FB (Feedback). Se houver upTempUp, faz recursivamente somando densidades.
template<typename S>
